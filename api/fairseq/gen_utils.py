...
def get_next(model, p, memory, has_prime = False):
    ...
    pr = torch.from_numpy(np.array(p))[None, None, :].to(_device)
...
def gen_one(model, prime_nums, MAX_LEN = 4090, MIN_LEN = 0, max_measures=None):
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(MAX_LEN - len(prime))):
            ...
            # Stop when a new measure-start token exceeds the target measure count
