...
if __name__ == '__main__':
    ...
    # Count measures in the full input MIDI so generation stays the same musical length
    full_prime, _ = process_prime_midi(sys.argv[1], 9999, 0)
    num_input_measures = sum(
        1 for tok in full_prime
        if music_dict.index2word(0, tok[0])[0].lower() == 'm'
    )
    print(f'Input has {num_input_measures} measures — generation will match this length.')
    ...
    for i in range(gen_cnt):
        while(True):
            try:
                generated, ins_logits = gen_one(m, prime, MIN_LEN = 0, max_measures = num_input_measures)
                break
